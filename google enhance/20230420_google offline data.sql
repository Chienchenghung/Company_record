...
select 
	...
	--,a.lead_sn
	,a.Name
	,convert(varchar(23),a.Time,126) as Time
	,a.Value
	,'TWD' as Currency
from (
	select isnull(b.lead_sn,a.lead_sn) as lead_sn,a.inpdate as Time,'NewLead' as Name,5738 as Value
	from bi_edw..FactMDInputLeadListClass as a with(nolock)
	left join bi_edw.DimSocialLead as b on a.SocialLeadId=b.SocialLeadId
	...
	union all
	---
	select isnull(b.lead_sn,a.lead_sn) as lead_sn,AssignDate as Time,'Assign' as Name,7650 as Value
	...
	union all
	---
	select lead_sn,reserved_time as Time,'Appointment' as Name,12750 as Value
	from bi_edw.uvFactDemo with(nolock)
	where reserved_time>=convert(date,DATEADD(DD,-90,GETDATE()))
	---
	union all
	---
	select lead_sn,DemoDT as Time,'Attendance' as Name,25500 as Value
	from bi_edw.uvFactDemo with(nolock)
