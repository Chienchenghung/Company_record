...
from (
		...
		from BI_EDW.FactMDAssignMediaTrace	as MDA with(nolock)
		where 1=1
--留名單 & 派發時間皆在預設時間範圍區間內
...
LEFT JOIN BI_EDW.DIMLEAD as Lead with(nolock)
	on MDA.lead_sn = Lead.lead_sn 

LEFT JOIN (
select YMD,SalesLevelID,SalesBrand, Team,sales_sn,ps.salesfname+' '+ps.saleslname as salesname ,ps2.*
	from BI_EDW.FactSalesRecordFromPS as record WITH(NOLOCK)  
	left join BI_EDW.FactSalesInfoFromPS as ps WITH(NOLOCK) on record.sales_sn=ps.IMSSalesSn
	left join (	select distinct DeptId,SalesRegionManagerEname,SalesRegionDescrCN,ManagerEname from BI_EDW.FactSalesInfoFromPS WITH(NOLOCK) where Location in (N'臺北',N'台中') and LeaveDate is null and DeptTW like N'%業務部%') as ps2 on record. PSDeptId = ps2.DeptId
...
INNER JOIN BI_EDW.DimMediaforTrace as Media with(nolock)
	on MDA.market_media_sn=Media.mediakey
	and L1_SN in (6,20)

LEFT JOIN bi_edw.DimSocialLead AS sl WITH (NOLOCK) 
       ...
       FROM   (
              SELECT 
                     ...
                     MAX( showTime) AS showTime,
                      db_source
              FROM    BI_EDW. FactMDDemoMediaTrace WITH (NOLOCK)
              GROUP BY
                      lead_sn,
                     MediaKey_FirstMedia ,
                     inpdate_FirstMedia ,
              ...
              SELECT  lead_sn,showTime, /*出席日期*/  status,	/*出席狀況*/ STATUS_byCon, db_source,MediaKey_FirstMedia as market_media_sn,
                     ROW_NUMBER() OVER(PARTITION BY lead_sn,db_source ORDER BY  ReservTime/*預約日期*/ DESC ) AS rn
              FROM    BI_EDW. FactMDDemoMediaTrace WITH (NOLOCK)
...
END;
    ...
    INTO #STV
    FROM BI_EDW.FactMDSTVMediaTrace AS stv  with(nolock)
...
SELECT case when BrandId = 1 then 'TutorABC(TP)' else 'tutorJr' end as 媒體品牌
	...
	,*		
	, YEAR(inpdate) as inpdate_Y
	, MONTH(inpdate) as  inpdate_M
...
left join 
(
	select team, COUNT(distinct sales_sn) as sales_cnt from bi_edw.FactSalesRecordFromPS with(nolock) where YMD=@sdate group by team
) as sales_record
on deptshort_first=team
...
and deptshort_first in (select  distinct DeptTW from bi_edw.FactSalesInfoFromPS with(nolock) where LeaveDate is null and (DeptTW like N'TBD_業務部_BD%' or DeptTW like N'業務部_DTS%')  )
group by 
 deptshort_first
 ,ismgm
 , sales_cnt
...
into #bind
from (select lead_sn,Deptshort_first,ismgm,AssignDate_first from #MDassigntoBDtrace where Inpdate >=@sdate and Inpdate <@edate and inpdate_Y = @Y and inpdate_M = @M) a
left join (select lead_sn,bindleadtime,BindLeadTime_new from  bi_edw.DimSocialLead  with(nolock)  where lead_sn in (select distinct lead_sn from #MDassigntoBDtrace)) as dsl
...
------------------------------------------------------
/*--TDate設定為撈取日期-1天，如要修改撈取日，則改此設定'即可---*/
SET @tdate = dateadd(m,-1,dateadd(day, -1, GETDATE())) ; --前月前一天
------------------------------------------------------
...
select 
		...
		--,count(case when isnull(BindLeadTime_new,bindleadtime)>AssignDate and isnull(BindLeadTime_new,bindleadtime)<=dateadd(dd,30,AssignDate) then 1 else null end) as bind_cnt_after30
into #bind_
from (select lead_sn,Deptshort_first,ismgm,AssignDate_first from #MDassigntoBDtrace_ where Inpdate >=@sdate and Inpdate <@edate and inpdate_Y = @Y and inpdate_M = @M) a
left join (select lead_sn,bindleadtime,BindLeadTime_new from  bi_edw.DimSocialLead  with(nolock)  where lead_sn in (select distinct lead_sn from #MDassigntoBDtrace_)) as dsl
	on a.lead_sn=dsl.lead_sn
group by 
		Deptshort_first
		,ismgm
...
select 
	...
	, #final1_.ismgm
	,派發數_last
	,sales_cnt as 期初人力_last
	,派發數_last*1.0/sales_cnt as 人均派發_last
	,預約數_last
	,應出席數_last
	,出席數_last
	,STV_last
...
select 
	...
	,case when SUM(期初人力)=0 then 0 else SUM(派發數)*1.0/SUM(期初人力) end as 人均派發_sum
...
select 
	...
	,MIN(成交率) as 成交率_min
	,MIN(綁定率) as 綁定率_min
	,MIN(人均派發) as 人均派發_min
	,max(預約率) as 預約率_max
	,max(出席率) as 出席率_max
	,max(成交率) as 成交率_max
	,max(綁定率) as 綁定率_max
	,max(人均派發) as 人均派發_max
into #final_this_mm
from #final_this
group by ismgm
...
select 
	...
	,case when SUM(期初人力_last)=0 then 0 else SUM(派發數_last)*1.0/SUM(期初人力_last) end as 人均派發_sum_last
